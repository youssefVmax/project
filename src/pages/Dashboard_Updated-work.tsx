...
export const Dashboard: React.FC = () => {
  ...
  const filterOptions = {
    sales_agent: getUniqueValues('sales_agent', 10), // Top 10 agents
  ...
  const topCloser = filteredData.reduce((max: SalesRow | null, item: SalesRow) =>{
    if (!max || item.amount_paid > max.amount_paid) {
      return item;
    }
    return max;
  ...
  // Ensure topAgent has correct type and fallback
const topAgent: { name: string; amount: number; deals: number } = topAgentData || { name: 'N/A', amount: 0, deals: 0 };
// Defensive: totalCommission is always a number
const totalCommission: number = filteredData.reduce((sum: number, item: SalesRow) => sum + (item.paid_per_month || 0), 0);
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {[
              ...
              { title: 'top closed', value: typeof topCloser === 'object' && topCloser !== null ? topCloser.customer_name || 'N/A' : String(topCloser), fullValue: `Only closed deals counted`, icon: TrendingUp, color: 'orange', trend: '+15%', deals: `100% closed` },
              { title: 'Top Agent', value: typeof topAgent === 'object' && topAgent !== null ? topAgent.name || 'N/A' : String(topAgent), fullValue: `$${topAgent.amount.toLocaleString()} • ${topAgent.deals} deals`, icon: Crown, color: 'pink', trend: 'Leader', deals: `${topAgent.deals} deals` }
            ].map((metric, index) => (
              <motion.div
                key={metric.title}
