  ...
  useEffect(() => {
    ...
    fetch(CSV_URL)
      ...
      .catch((error) => {
        ...
        console.error("Failed to load CSV file. Please ensure the file exists at:", CSV_URL)
        setSalesData([])
        setFilteredData([])
        setLoading(false)
      })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Data Loading Failed</h2>
          <p className="text-slate-600 dark:text-slate-400 mb-4">
            Unable to load the CSV data file. Please ensure the file "Three-month-dashboard-R.csv" is placed in the "public/data/" folder of your project.
          </p>
          <p className="text-sm text-slate-500 dark:text-slate-500">
            Expected file path: /data/Three-month-dashboard-R.csv
          </p>
        </div>
      </div>
    )
